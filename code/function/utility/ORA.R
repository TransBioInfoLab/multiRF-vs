...
ORA <- function(gene_list, pathway = "KEGG", method = "ORA", ...){
  ...
  if (pathway == "KEGG"){
    ...
    pathway.df <- c5.bp %>% dplyr::select(gs_name, gene_symbol)
  } else if (pathway == "c7.IMMUNESIGDB") {
    c7.IMMUNESIGDB <- msigdbr(species = "Homo sapiens", category = "C7", subcategory = "IMMUNESIGDB")
    pathway.df <- c7.IMMUNESIGDB %>% dplyr::select(gs_name, gene_symbol)
  } else if (pathway == "H"){
    h <- msigdbr(species = "Homo sapiens", category = "H")
    pathway.df <- h %>% dplyr::select(gs_name, gene_symbol)
  } else if (pathway == "c2.cp") {
    p1 <- msigdbr(species = "Homo sapiens", category = "C2", subcategory = "KEGG_MEDICUS")
    ...
    pathway.df <- rbind(p1,p2,p3,p4,p5)  %>% dplyr::select(gs_name, gene_symbol)
  } else {
...
plot_pathway <- function(results, topn = 10, ylim = 1e-5, padj = 0.05) {
  ...
  results <- results %>%
    ...
    mutate(logp = -log10(pvalue),
           ...
           Description = gsub("GOBP |REACTOME |WP |PID |BIOCARTA |KEGG ", "", Description))
